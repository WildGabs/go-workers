...
func FetchSpec(c gospec.Context) {
	...
	c.Specify("Fetch", func() {
		...
		c.Specify("it puts messages from the queues on the messages channel", func() {
			...
			conn.LPush(ctx, "queue:fetchQueue2", message.ToJson())
			...
			len, _ := conn.LLen(ctx, "queue:fetchQueue2").Result()
		...
		c.Specify("places in progress messages on private queue", func() {
			...
			conn.LPush(ctx, "queue:fetchQueue3", message.ToJson())
			fetch.Ready() <- true
			<-fetch.Messages()

			len, _ := conn.LLen(ctx, "queue:fetchQueue3:1:inprogress").Result()
			c.Expect(len, Equals, int64(1))

			messages, _ := conn.LRange(ctx, "queue:fetchQueue3:1:inprogress", 0, -1).Result()
		...
		c.Specify("removes in progress message when acknowledged", func() {
			...
			conn.LPush(ctx, "queue:fetchQueue4", message.ToJson())
			...
			len, _ := conn.LLen(ctx, "queue:fetchQueue4:1:inprogress").Result()
			c.Expect(len, Equals, int64(0))
		...
		c.Specify("removes in progress message when serialized differently", func() {
			...
			conn.LPush(ctx, "queue:fetchQueue5", json)
			...
			len, _ := conn.LLen(ctx, "queue:fetchQueue5:1:inprogress").Result()
			c.Expect(len, Equals, int64(0))
		...
		c.Specify("refires any messages left in progress from prior instance", func() {
			...
			conn.LPush(ctx, "queue:fetchQueue6:1:inprogress", message.ToJson())
			conn.LPush(ctx, "queue:fetchQueue6:1:inprogress", message2.ToJson())
			conn.LPush(ctx, "queue:fetchQueue6", message3.ToJson())
			...
			len, _ := conn.LLen(ctx, "queue:fetchQueue6:1:inprogress").Result()
			c.Expect(len, Equals, int64(0))
